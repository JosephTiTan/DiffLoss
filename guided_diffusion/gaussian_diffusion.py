...
result_path = "./DiffLoss/result"
best_path = "./DiffLoss/result/best"
file_path = "./DiffLoss/result/log.txt"
save_path = "./DiffLoss/save"
...
def copy_files(source_folder, destination_folder):
    ...
    for file_name in files:
        ...
        if source_file.endswith(".png"):
            ...
            # 保存图像到目标文件夹
            cv2.imwrite(destination_file, cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
...
class GaussianDiffusion:
    ...
    # def save_figs(self, img, t, name):
    #     sample = ((img + 1) * 127.5).clamp(0, 255).to(th.uint8)
    #     sample = sample.permute(0, 2, 3, 1)
    #     sample = sample.contiguous()
    #     sample = sample[0].cpu().numpy()
    #     path = ""
    #     # path = os.path.join(path, "{}".format(int(t)))
    #     if not os.path.exists(path):
    #         os.makedirs(path)
    #     out_path = os.path.join(path, f"{int(t)}_{name}.png")
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        ...
        # self.save_figs(pred_xstart, t, "x0")
    ...
    def condition_score(self, cond_fn, p_mean_var, x, t, model_kwargs=None):
        ...
        if self.correcting_x0_fn is not None:
            out["pred_xstart"] = self.correcting_x0_fn(out["pred_xstart"])
    ...
    def ddim_sample(
        ...
        sigma = (eta * th.sqrt((1 - alpha_bar_prev) / (1 - alpha_bar)) * th.sqrt(1 - alpha_bar / alpha_bar_prev))   # sigma = 0
        # Equation 12.
        noise = th.randn_like(x)
    ...
    def ddim_sample_loop(
        ...
        if device is None:
            device = next(model.parameters()).device
        output = []
        img = th.randn(*shape, device=device).cuda()
        noise = img.clone()
        indices = list(range(self.num_timesteps))[::-1]
        ...
        for i in tqdm(indices):
            ...
            with th.no_grad():
                ...
                img = out["sample"]
                pre_x0 = out["pred_xstart"]
                pre_noise = out["predicted_noise"]
    ...
    def training_losses(self, restoration_model, model, degrad, gt, t, step, val_loader, model_kwargs=None, noise=None):
        terms = {}
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
        ...
        output = restoration_model((degrad+1)/2.0)
        output = output * 2 - 1
        
        output_t = self.q_sample(output, t, noise=noise)
        gt_t = self.q_sample(gt, t, noise=noise)

        with torch.no_grad():
            output_noise, output_hspace = model(output_t, self._scale_timesteps(t), **model_kwargs)
            gt_noise, gt_hspace = model(gt_t, self._scale_timesteps(t), **model_kwargs)

        B, C = gt_t.shape[:2]
        output_noise, _ = th.split(output_noise, C, dim=1)
        gt_noise, _ = th.split(gt_noise, C, dim=1)

        terms["spatial_loss"] = mean_flat((output - gt) ** 2)
        terms["diffusion_loss"] = 0.001 * mean_flat((output_noise - gt_noise) ** 2)
        terms["hspace_loss"] = 0.000001 * mean_flat((output_hspace - gt_hspace) ** 2)
        terms["loss"] = terms["spatial_loss"] + terms["hspace_loss"] + terms["diffusion_loss"]
        ...
        if step % 30 == 0:
            ...
            with torch.no_grad():
                for i, val_data in enumerate(val_loader):
                    idx += 1
                    ro_degrad, ro_gt = val_data
                    ro_degrad = ro_degrad.to(dist_util.dev())
                    ro_gt = ro_gt.to(dist_util.dev())

                    ro_output = restoration_model((ro_degrad+1)/2.0) # (0,1)
                    ro_output = ro_output * 2 - 1  # (-1,1)

                    loss = L2lloss(ro_gt, ro_output)

                    ro_output = (ro_output + 1)   # (0,2)
                    ro_gt = (ro_gt + 1)
                    ro_degrad = (ro_degrad + 1)

                    out_dict['Out'] = ro_output.detach().float().cpu()
                    out_dict['HR'] = ro_gt.detach().float().cpu()
                    out_dict['LR'] = ro_degrad.detach().float().cpu()
                    print('Detach finish')
            ...
            if self.bast_psnr < avg_psnr:
                self.bast_psnr = avg_psnr
                torch.save({'net': restoration_model.state_dict()},
                           os.path.join(save_path, 'IAT01best_net.pth'))
                copy_files(result_path, best_path)
