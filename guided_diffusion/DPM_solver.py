    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        ...
        with torch.no_grad():
            if method == 'adaptive':
                ...
                num_rows = 2
                num_cols = 5
                fig, axs = plt.subplots(num_rows, num_cols, figsize=(15, 5))  # 可以根据需要调整figsize参数
                for i, tensor in enumerate(self.x_t):
                    tensor_flattened = tensor.view(-1).cpu().numpy()
                    ax = axs[i // num_cols, i % num_cols]
                    ax.hist(tensor_flattened, bins=200, range=(-4, 4))
                    ax.set_title(f'Histogram (t={9-i})')
                    ax.set_xlabel('Value')
                    ax.set_ylabel('Frequency')
                plt.tight_layout()
                plt.savefig('/mnt/workspace/workgroup/yuhu/code/guided-diffusion/samples/x_t.png')
                plt.close()
                ...
                num_cols = 5
                fig, axs = plt.subplots(num_rows, num_cols, figsize=(15, 5))  # 可以根据需要调整figsize参数
                for i, tensor in enumerate(self.x_0):
                    tensor_flattened = tensor.view(-1).cpu().numpy()
                    ax = axs[i // num_cols, i % num_cols]
                    ax.hist(tensor_flattened, bins=200, range=(-4, 4))
                    ax.set_title(f'Histogram (t={9-i})')
                    ...
                    ax.set_ylabel('Frequency')
                plt.tight_layout()
                plt.savefig('/mnt/workspace/workgroup/yuhu/code/guided-diffusion/samples/x_0.png')
                plt.close()
                ...
                num_cols = 5
                fig, axs = plt.subplots(num_rows, num_cols, figsize=(15, 5))  # 可以根据需要调整figsize参数
                for i, tensor in enumerate(self.noise):
                    tensor_flattened = tensor.view(-1).cpu().numpy()
                    ax = axs[i // num_cols, i % num_cols]
                    ax.hist(tensor_flattened, bins=200, range=(-4, 4))
                    ax.set_title(f'Histogram (t={9-i})')
                    ...
                    ax.set_ylabel('Frequency')
                plt.tight_layout()
                plt.savefig('/mnt/workspace/workgroup/yuhu/code/guided-diffusion/samples/noise.png')
                plt.close()
                
                # import matplotlib.pyplot as plt
                # import numpy as np
                # x1 = np.linspace(0,10,10)  # 创建从1到10的等间隔数列
                # y1 = self.var
                # y2 = self.mean
                # plt.plot(x1, y1, label='eps', color='green',linewidth=2,linestyle=':')
                # plt.plot(x1, y2, label='D1', color='red',linewidth=2,linestyle='-')
                # plt.title('T = 10 (s=16)')
                # plt.xlabel('t')
                # plt.ylabel('variance')
                # plt.legend(prop = {'size':10})
                # plt.savefig("/mnt/workspace/workgroup/yuhu/code/guided-diffusion/samples/4.png")
